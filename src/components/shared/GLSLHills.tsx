...
const GLSLHills = ({
  ...
  useEffect(() => {
          ...
          new THREE.RawShaderMaterial({
            ...
            vertexShader: `
              ...
              void main(void) {
                ...
                float noise1 = cnoise(np*0.08);
                float noise2 = cnoise(np*0.06);
                float noise3 = cnoise(np*0.4);
                vec3 lp = p + vec3(0.0,
                  noise1*s1*8.0 + noise2*s1*8.0 + noise3*(abs(s1)*2.0+0.5) + pow(s1,2.0)*40.0,
                  0.0);
                vPosition = lp;
            ...
            fragmentShader: `
              ...
              void main(void) {
                ...
                vec3 color = h < 0.5
                  ...
                  : mix(colMid, colHigh, (h - 0.5) * 2.0);

                // Distance-based edge fade
                float fade = clamp((96.0 - length(vPosition)) / 180.0, 0.0, 1.0);
    ...
    try {
      ...
      // WebGL unavailable (GPU disabled / sandboxed browser) — CSS fallback will show
      console.warn('[GLSLHills] WebGL unavailable — showing CSS fallback');
      return;
    }
  ...
  return (
    <div
      ref={containerRef}
      style={{ position: 'absolute', inset: 0, width, height, zIndex: 0 }}
    >
      <canvas
        ref={canvasRef}
